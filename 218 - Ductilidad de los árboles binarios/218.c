...
void averiguaPostOrden(vector<int> &inorden, int a, int b, vector<int>&postorden, int c, int d, vector<int>&preorden) {
	if (a > b) {}
	else if (b - a == 0) {
		preorden.push_back(postorden[d]);
	}
	else {
		preorden.push_back(postorden[d]);
		int aux=a;
		while (inorden[aux] != postorden[d]) {
			aux++;
		}
		averiguaPostOrden(inorden, a, aux - 1, postorden, c, c + aux - a - 1, preorden);
		averiguaPostOrden(inorden, aux + 1, b, postorden, c + aux - a, d - 1, preorden);
	}
...
bool resuelveCaso() {
	...
	averiguaPostOrden(inorden, 0, inorden.size() - 1,postorden, 0, postorden.size() - 1, preorden);
