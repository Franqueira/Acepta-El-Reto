...
bintree<char> codificaEspejo(bintree<char> arbol) {
	if (arbol.empty()) { return{}; }
	auto iz = codificaEspejo(arbol.left());
	auto der=codificaEspejo(arbol.right());
	return{ der,arbol.root(),iz };
...
bool resuelveCaso() {
	...
	if (!cin)return false;
	auto sol = codificaEspejo(arbol);
	vector<char> solu = sol.preorder();
