...
bintree<char> construyeArbol(string &g) {
	char c = g[g.size() - 1];
	g.pop_back();
	if (isdigit(c))	return { c };
	else { 
		bintree<char> derecho = construyeArbol(g);
		return{ construyeArbol(g),c,derecho }; 
	}
}
 ...
 bool resuelveCaso() {
	 ...
	 getline(cin, g);
	 if (!cin)return false;
	 bintree<char> arbol = construyeArbol(g);
	 vector<char> cola = arbol.levelorder();
